...
export default function ChoCounsche({ route }) {
  ...
  async function fetchSchedules() {
    ...
    try {
      ...
      // Separate patient IDs into regular patients and AP patients
      const regularPatientIds = [];
      const apPatientIds = [];
      
      meetingsData.forEach(m => {
        if (m.pat_id && m.pat_id.startsWith("AP")) {
          apPatientIds.push(m.pat_id);
        } else {
          regularPatientIds.push(m.pat_id);
        }
      });

      const patientMap = {};
      const ashaPatientMap = {};

      // Fetch regular patient info
      if (regularPatientIds.length > 0) {
        let { data: patientsData, error: patientsError } = await supabase
          ...
          .in("patient_no", regularPatientIds);

        if (patientsError) {
          console.error("Failed to fetch regular patients:", patientsError.message);
        } else if (patientsData) {
          patientsData.forEach((patient) => {
            patientMap[patient.patient_no] = {
              ...patient,
              type: "regular"
            };
          });
      ...
      // Fetch ASHA/Aid register patient info for AP patients
      if (apPatientIds.length > 0) {
        let { data: ashaPatientsData, error: ashaPatientsError } = await supabase
          .from("asha_aidregis")
          .select("p_id, p_name, p_age, p_gender, p_phone, asha_name, asha_id")
          .in("p_id", apPatientIds);

        if (ashaPatientsError) {
          console.error("Failed to fetch ASHA patients:", ashaPatientsError.message);
        } else if (ashaPatientsData) {
          ashaPatientsData.forEach((patient) => {
            ashaPatientMap[patient.p_id] = {
              name: patient.p_name,
              age: patient.p_age,
              gender: patient.p_gender,
              phone_no: patient.p_phone,
      ...
      // Merge patient info into meetings
      const combinedMeetings = meetingsData.map((meeting) => {
        if (meeting.pat_id && meeting.pat_id.startsWith("AP")) {
          // This is an ASHA patient
          return {
            ...meeting,
            patientInfo: ashaPatientMap[meeting.pat_id] || null,
            patientType: "asha"
          };
        } else {
          // This is a regular patient
          return {
            ...meeting,
          ...
          <FlatList
            ...
            renderItem={({ item }) => (
              <View style={styles.meetingCard}>
                ...
                {/* Display ASHA/ANM Worker info for AP patients */}
                {item.patientType === "asha" && item.patientInfo?.asha_name && (
                  <Text style={styles.ashaDetail}>
                    ASHA/ANM Worker: {item.patientInfo.asha_name} ({item.patientInfo.asha_id})
                  </Text>
                )}
                
                <Text style={styles.patientDetail}>
                  Name: {item.patientInfo?.name || item.patientInfo?.p_name || "Patient not found"}
                </Text>
                <Text style={styles.patientDetail}>
                  Age: {item.patientInfo?.age || item.patientInfo?.p_age || "N/A"}
                </Text>
                <Text style={styles.patientDetail}>
                  Gender: {item.patientInfo?.gender || item.patientInfo?.p_gender || "N/A"}
                </Text>
                <Text style={styles.patientDetail}>
                  Phone: {item.patientInfo?.phone_no || item.patientInfo?.p_phone || "N/A"}
                </Text>
