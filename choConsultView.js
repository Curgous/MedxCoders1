...
export default function ChoConsultView({ navigation, route }) {
    ...
    const renderConsultCard = (consult, showActions = false) => {
        ...
        const isAshaAided = consult.asha_aid === "Yes";
        ...
        return (
            <View key={consult.id} style={styles.card}>
                ...
                {/* ASHA Aided indicator - top left corner */}
                {isAshaAided && (
                    <View style={styles.ashaIndicator}>
                        <Text style={styles.ashaIndicatorText}>ASHA aided</Text>
                    </View>
                )}
                
                {/* Conditionally set top margin based on ASHA indicator presence */}
                <View style={[
                    styles.basicInfo,
                    isAshaAided ? styles.basicInfoWithAsha : styles.basicInfoWithoutAsha
                ]}>
                    <View style={styles.nameRow}>
                        <Text style={styles.patientName}>{consult.patient_name || 'N/A'}</Text>
                    </View>
                    
                    {/* Display ASHA name and ID for ASHA aided patients */}
                    {isAshaAided && consult.asha_name && (
                        <Text style={styles.ashaDetail}>
                            {consult.asha_name} ({consult.asha_id})
                        </Text>
                    )}
                    
                    {/* Hide Reason for ASHA aided patients */}
                    {!isAshaAided && (
                ...
                {isExpanded && (
                    <View style={styles.expandedDetails}>
                        ...
                        {/* Different display for ASHA aided vs regular patients */}
                        {isAshaAided ? (
                            // ASHA Aided Patient Details
                            <View style={styles.detailColumn}>
                                <Text style={styles.detailLabel}>Reported Symptoms:</Text>
                                <Text style={styles.detailValue}>
                                    {consult.asha_symptoms || 'N/A'}
                                </Text>
                            </View>
                        ) : (
            ...
            user: {
                ...
                asha_id: consult.asha_id // Add ASHA ID
                        ...
                        {/* Hide Generate Report button for ASHA aided patients */}
                        {!isAshaAided && (
                            <TouchableOpacity
...
const styles = StyleSheet.create({
    ...
    basicInfo: {
        marginBottom: 12,
        // Base styles - individual marginTop will be set conditionally
    },
    basicInfoWithAsha: {
        marginTop: 25, // More space when ASHA indicator is present
    },
    basicInfoWithoutAsha: {
        marginTop: 10, // Less space when no ASHA indicator
    },
    nameRow: {
        flexDirection: 'row',
        alignItems: 'center',
