...
export default function DocEmer({ navigation,route }) {
  ...
  const handleNotifyDoctorCho = async (emerId, phoneNumber) => {
    if (!phoneNumber) {
      Alert.alert('Error', 'No phone number available for this ASHA worker');
      return;
    }
    ...
    <View key={alert.emer_id} style={styles.alertBox}>
      ...
      <View style={styles.actionButtonContainer}>
        {showAcceptButton ? (
          ...
          <>
            {/* Call ASHA Worker Button */}
            <TouchableOpacity
              style={[
                styles.notifyButton,
                !alert.assigned_ashaphone && styles.disabledButton
              ]}
              onPress={() => handleNotifyDoctorCho(alert.emer_id, alert.assigned_ashaphone)}
              disabled={!alert.assigned_ashaphone}
            >
              <Text style={styles.buttonText}>
                {alert.assigned_ashaphone ? 'Call ASHA Worker' : 'ASHA Phone Unavailable'}
              </Text>
            </TouchableOpacity>
            ...
            >
              <Text style={styles.buttonText}>
                {alert.p_phone ? 'Call Patient' : 'Patient Phone Unavailable'}
              </Text>
            </TouchableOpacity>
          </>
        )}
