...
export default function PharmacistSignup({ navigation }) {
  ...
  const handleSignup = async () => {
    if (!pName.trim() || !phone.trim() || !password.trim() || !confirmPassword.trim()) {
    ...
    try {
      ...
      Alert.alert("Account Created", `ID: ${pharmacist_id}\nSignup successful!`);
      setPName("");
      setPhone("");
      setPassword("");
      setConfirmPassword("");
      // navigation.navigate('PharmacistLogin'); // Optional: navigate to login
    } catch (err) {
